...
class Monetta {
  ...
  login() {
    return async (req, res, next) => {
      try {
        ...
        if (!user) {
          next(new createError.Unauthorized('User not found'));
          return;
        }
  ...
  async getTokenEntity(accessToken) {
    if (!accessToken) {
      throw new createError.Unauthorized('Please provide access token');
    }
    const tokenEntity = await this.getAccessToken({ accessToken });
    if (!tokenEntity) {
      throw new createError.Unauthorized('Token is not valid');
    }
    return tokenEntity;
  }
  ...
  async _checkTokenNumberExceeded(user) {
    ...
    if (activeAccessTokens > maxTokensAllowed) {
      throw new createError.Forbidden('Too many active access tokens.');
    }
  }
  ...
  _getUserMainFieldQuery(body) {
    ...
    if (!mainFieldValue) {
      throw new createError.BadRequest(`Please provide ${usersMainField}`);
    }
    return {
  ...
  _checkPasswordHashes(body, user) {
    ...
    if (!password) {
      throw new createError.BadRequest(`Please provide ${passwordField}`);
    }
